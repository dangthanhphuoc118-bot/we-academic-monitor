...
import { sampleSpeakingQuestions } from "@/lib/speaking-questions";
...
export function CurriculumStudentCheck({ classes, students, curriculum, levelOptions, feedbackOptions, selectedClassId, setSelectedClassId, selectedStudentId, setSelectedStudentId, onSaved, openFeedbackOptions, canManage = true, embedded = false, editingCheck = null, initialCheckedAt }: StudentCheckProps) {
  ...
  const [freestyleQuestions, setFreestyleQuestions] = useState<string[]>(savedFreestyleQuestions.length ? savedFreestyleQuestions : program?.group === "cambridge" ? sampleSpeakingQuestions(unit?.freestyleQuestions || []) : []);
  const [saving, setSaving] = useState(false);

  const resetEvaluation = (questionBank = unit?.freestyleQuestions || []) => {
    ...
    setFreestyleQuestions(program?.group === "cambridge" ? sampleSpeakingQuestions(questionBank) : []);
  };
  ...
  const complete = program?.group === "baby" ? spellingPercent !== "" && writingPercent !== "" : program?.group === "super" ? vocabularyCorrect !== "" && communicationPercent !== "" && Boolean(pronunciation) : patternPercent !== "" && freestylePercent !== "" && Boolean(pronunciation) && Boolean(oneOrMany) && Boolean(amIsAre);
  ...
  return <div className="space-y-6">
    ...
    {student && program && unit ? <div className="grid gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(340px,.75fr)]">
      <div className="space-y-5"><CurriculumReference unit={unit} /><Card className="border-0 shadow-[0_10px_30px_rgba(18,48,67,0.07)]"><CardHeader className="border-b"><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="text-xl">Evaluation Criteria</CardTitle><p className="mt-1 text-sm text-muted-foreground">{student.name} · {classInfo?.name}</p></div><ProgramBadge group={program.group}>{program.label}</ProgramBadge></div></CardHeader><CardContent className="space-y-4 p-5">
        ...
        {program.group === "cambridge" ? <><div className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">COMMUNICATION</div><div className="rounded-xl border bg-white p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold">FREESTYLE QUESTIONS</p><p className="mt-1 text-xs text-muted-foreground">5 câu được chọn ngẫu nhiên từ ngân hàng của {unit.unitLabel} trong Khung chương trình.</p></div><Button type="button" size="sm" variant="outline" onClick={() => setFreestyleQuestions(sampleSpeakingQuestions(unit.freestyleQuestions))}><RefreshCw /> Đổi 5 câu</Button></div><ol className="mt-3 space-y-2 pl-5 text-sm leading-6">{freestyleQuestions.map((question, index) => <li key={`${question}-${index}`} className="list-decimal">{question}</li>)}</ol></div><div className="grid gap-3 sm:grid-cols-2"><NumberScore id="pattern" label="PATTERN" value={patternPercent} onChange={setPatternPercent} /><NumberScore id="freestyle" label="FREESTYLE" value={freestylePercent} onChange={setFreestylePercent} /></div><PronunciationChoice value={pronunciation} onChange={setPronunciation} /><div className="grid gap-3 sm:grid-cols-2"><BinaryChoice label="ONE OR MANY" value={oneOrMany} onChange={setOneOrMany} /><BinaryChoice label="AM – IS – ARE" value={amIsAre} onChange={setAmIsAre} /></div></> : null}
...
export function CurriculumManager({ curriculum, onReload }: { curriculum: CurriculumUnit[]; onReload: () => Promise<void> | void }) {
  ...
  return <div className="space-y-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="mb-1 text-xs font-bold uppercase tracking-[0.15em] text-[#d95c25]">Curriculum Library</p><h1 className="text-2xl font-bold tracking-tight sm:text-3xl">Nội dung khung đánh giá</h1><p className="mt-1 max-w-3xl text-sm leading-6 text-muted-foreground">Toàn bộ nội dung từ 12 tài liệu đã được đưa vào hệ thống. Vocabulary, Grammar / Communication và ngân hàng Freestyle theo từng Unit được hiển thị riêng để xem và chỉnh sửa.</p></div><div className="w-full sm:w-72"><Label className="mb-2">Chương trình</Label><Picker value={programCode} onChange={setProgramCode} placeholder="Chọn chương trình" options={programs.map((program) => ({ value: program.code, label: curriculum.find((unit) => unit.programCode === program.code)?.programLabel || program.label }))} /></div></div>
...
function CurriculumEditor({ unit, onClose, onSaved }: { unit: CurriculumUnit; onClose: () => void; onSaved: () => Promise<void> | void }) {
  ...
  const [freestyleText, setFreestyleText] = useState(unit.freestyleQuestions.join("\n"));
  const [vocabularyMax, setVocabularyMax] = useState(String(unit.vocabularyMax));
  const [saving, setSaving] = useState(false);
  const freestyleQuestions = Array.from(new Set(freestyleText.split(/\r?\n/).map((question) => question.trim()).filter(Boolean)));

  const save = async () => {
    if (unit.group === "cambridge" && freestyleQuestions.length < 5) {
      toast.error("Vui lòng nhập ít nhất 5 câu Freestyle, mỗi câu một dòng.");
      return;
    }
    setSaving(true);
    try {
      await send("updateCurriculumUnit", { programCode: unit.programCode, unitNumber: unit.unitNumber, topic, content, vocabulary, grammar, freestyleQuestions, vocabularyMax: Number(vocabularyMax), writingRef: "" });
      await onSaved();
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Chỉnh sửa {unit.programLabel} · {unit.unitLabel}</DialogTitle><DialogDescription>Nội dung chỉnh sửa sẽ thay thế bản gốc khi giáo viên mở phiếu đánh giá. Với Movers và Flyers, giữ định dạng mỗi dòng bắt đầu bằng ADJ, NOUN, VERB, ADV hoặc PREPOSITION.</DialogDescription></DialogHeader><div className="grid gap-4 py-2">
    ...
    {unit.group === "cambridge" ? <Field label={`Freestyle question bank · ${freestyleQuestions.length} câu`}><p className="text-xs leading-5 text-muted-foreground">Nhập mỗi câu trên một dòng. Phiếu kiểm tra sẽ chọn ngẫu nhiên 5 câu từ riêng Unit này.</p><Textarea value={freestyleText} onChange={(event) => setFreestyleText(event.target.value)} rows={14} maxLength={100000} placeholder="Nhập ít nhất 5 câu, mỗi câu một dòng..." /></Field> : null}
