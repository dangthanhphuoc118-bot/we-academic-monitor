...
function CurriculumEditor({ unit, onClose, onSaved }: { unit: CurriculumUnit; onClose: () => void; onSaved: () => Promise<void> | void }) {
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Chỉnh sửa {unit.programLabel} · {unit.unitLabel}</DialogTitle><DialogDescription>Nội dung chỉnh sửa sẽ thay thế bản gốc khi giáo viên mở phiếu đánh giá. Với Movers và Flyers, giữ định dạng mỗi dòng bắt đầu bằng ADJ, NOUN, VERB, ADV hoặc PREPOSITION.</DialogDescription></DialogHeader><div className="grid gap-4 py-2"><Field label="Chủ đề"><Input value={topic} onChange={(event) => setTopic(event.target.value)} /></Field><Field label="Nội dung"><Input value={content} onChange={(event) => setContent(event.target.value)} /></Field><Field label="Vocabulary">{unit.programCode === "FLYERS" && sourceVocabulary && vocabulary !== sourceVocabulary ? <div className="space-y-2 rounded-lg bg-emerald-50 p-3"><p className="text-xs text-emerald-800">Nạp 5 nhóm từ loại từ PDF Flyers mới. Các trường khác giữ nguyên; chỉ cập nhật khi bấm Lưu nội dung.</p><Button type="button" size="sm" variant="outline" onClick={() => setVocabulary(sourceVocabulary)}><RefreshCw /> Nạp vocab Flyers mới</Button></div> : null}<Textarea value={vocabulary} onChange={(event) => setVocabulary(event.target.value)} rows={10} /></Field><Field label={unit.group === "cambridge" ? "Communication guidance" : "Grammar / Communication"}><Textarea value={grammar} onChange={(event) => setGrammar(event.target.value)} rows={5} /></Field><Field label="Tổng số từ vựng"><Input type="number" min={0} value={vocabularyMax} onChange={(event) => setVocabularyMax(event.target.value)} /></Field></div><DialogFooter><Button variant="outline" onClick={onClose}>Hủy</Button><Button onClick={save} disabled={saving}>{saving ? <LoaderCircle className="animate-spin" /> : <Save />} Lưu nội dung</Button></DialogFooter></DialogContent></Dialog>;
}
