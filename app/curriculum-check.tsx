...
import { curriculumKey, programByLabel, programs, type CurriculumUnit, type LearningCheck, type ProgramGroup } from "@/lib/curriculum";
...
export function CurriculumStudentCheck({ classes, students, curriculum, levelOptions, feedbackOptions, selectedClassId, setSelectedClassId, selectedStudentId, setSelectedStudentId, onSaved, openFeedbackOptions, canManage = true, embedded = false, editingCheck = null }: StudentCheckProps) {
  ...
  const [checkedAt, setCheckedAt] = useState(editingCheck?.checkedAt || new Date().toISOString().slice(0, 10));
  ...
  const [freestyleQuestions, setFreestyleQuestions] = useState<string[]>(savedFreestyleQuestions.length ? savedFreestyleQuestions : program?.group === "cambridge" ? sampleSpeakingQuestions(program.code) : []);
  ...
  const resetEvaluation = () => {
    ...
    setFreestyleQuestions(program?.group === "cambridge" ? sampleSpeakingQuestions(program.code) : []);
  };
...
const vocabularyGroupLabels = ["ADJ", "NOUN", "VERB", "ADV", "PREPOSITION"] as const;

function parseVocabularyGroups(content: string) {
...
function CurriculumEditor({ unit, onClose, onSaved }: { unit: CurriculumUnit; onClose: () => void; onSaved: () => Promise<void> | void }) {
  const [topic, setTopic] = useState(unit.topic); const [content, setContent] = useState(unit.content); const [vocabulary, setVocabulary] = useState(unit.vocabulary); const [grammar, setGrammar] = useState(unit.grammar); const [vocabularyMax, setVocabularyMax] = useState(String(unit.vocabularyMax)); const [saving, setSaving] = useState(false);
  ...
  return <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Chỉnh sửa {unit.programLabel} · {unit.unitLabel}</DialogTitle><DialogDescription>Nội dung chỉnh sửa sẽ thay thế bản gốc khi giáo viên mở phiếu đánh giá. Với Movers, giữ định dạng mỗi dòng bắt đầu bằng ADJ, NOUN, VERB, ADV hoặc PREPOSITION.</DialogDescription></DialogHeader><div className="grid gap-4 py-2"><Field label="Chủ đề"><Input value={topic} onChange={(event) => setTopic(event.target.value)} /></Field><Field label="Nội dung"><Input value={content} onChange={(event) => setContent(event.target.value)} /></Field><Field label="Vocabulary"><Textarea value={vocabulary} onChange={(event) => setVocabulary(event.target.value)} rows={10} /></Field><Field label={unit.group === "cambridge" ? "Communication guidance" : "Grammar / Communication"}><Textarea value={grammar} onChange={(event) => setGrammar(event.target.value)} rows={5} /></Field><Field label="Tổng số từ vựng"><Input type="number" min={0} value={vocabularyMax} onChange={(event) => setVocabularyMax(event.target.value)} /></Field></div><DialogFooter><Button variant="outline" onClick={onClose}>Hủy</Button><Button onClick={save} disabled={saving}>{saving ? <LoaderCircle className="animate-spin" /> : <Save />} Lưu nội dung</Button></DialogFooter></DialogContent></Dialog>;
}
